...
load_dotenv()
logging.basicConfig(filename='logs.txt', level=logging.DEBUG,format='%(asctime)s %(levelname)s %(message)s', datefmt='%m/%d/%Y %I:%M:%S %p')
...
def sendMail(mail_user,data_tuple):
    try :
        ...
        password = os.getenv('email_password') #input("Type your password and press enter:")

        message = MIMEMultipart("alternative")
        message["Subject"] = "New Games !"
...
def table_data(my_tuple):
    ...
    for i in range (len(my_tuple)):
        row +="<tr>"
        row += f"<td><img src={my_tuple[i][1]}></td>"
...
def htmlPage (my_tuple):
    ...
    <html>
            <head>
            <h1 style="text-align: left;">Lists of new games</h1>
